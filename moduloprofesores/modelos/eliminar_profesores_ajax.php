...
if (isset($_GET['txtSearch']) && $_GET['txtSearch'] != '') {
	...
	$item = "%" . $txtSearch . "%";
	$sql = "SELECT login FROM profesores as suggest1 where (login like '$item') or (nombre like '$item')";
} else if ($_GET["txtSearch"] == "")//el us no introduce nada en el input text
	$sql = "select login from profesores as suggest1 where login not LIKE '%'";
