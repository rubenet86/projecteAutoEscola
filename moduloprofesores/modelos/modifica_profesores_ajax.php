...
$sql="SELECT * FROM profesores order by $criterio_ord ".$orden." ".$mas;
if($_GET["text"]!=""){
	$text="%".$_GET["text"]."%";
	$sql="SELECT * FROM profesores where login like '$text' order by $criterio_ord ".$orden." ".$mas;
}
else if($_GET["text"]=="")//el us no introduce nada en el input text
	$sql = "select * from profesores where login not LIKE '%'";
...
try{
	...
	if ($resultado->num_rows > 0){
		$cadena.= '<br>';$cadena.= '<br>';$cadena.= '<br>';$cadena.= '<br>';
			$cadena.='<form name="formulario" action="../controlador/index.php?controlador=profesores&accion=recogeDatosModifica" method="post">';
			...
			while ($obj = $resultado->fetch_object()){
				...
				$cadena.='<tr>';
					...
					$cadena.='<td align="center"><input type="text" name="sexo" size="13" value='.$obj->sexo.' onkeyup="valida_envia(this)"></td>'; 
				$cadena.='</tr>';
			} 
			$cadena.='</table>';
			if($resultado->num_rows==1)
					$cadena.= '<input type="button" id="botoModifica" name="modifica" value="Modifica" onclick="enviar()">';	
			$cadena.='<form>';
